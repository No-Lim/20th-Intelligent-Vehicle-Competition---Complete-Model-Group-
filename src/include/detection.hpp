...
std::vector<PredictResult> com_predictor_results;

class Detection
{
public:
    std::vector<PredictResult> results; // AI推理结果
    float score = 0.45;                  // AI检测置信度
    ...
    void render()
    {
        ...
        for (int i = 0; i < prod; i += 6)
        {
            ...
            // turnning....
            if (result.type < (int)labels.size())
    ...
    void drawBox(Mat &img)
    {
        for (size_t i = 0; i < results.size(); i++)
        {
            ...
            //std::cout <<"drawBOX检测模型:"<<" "<<result.label<<std::endl;
        }
    }
   ...
   std::vector<PredictResult> getPredictResult()
   {
...
struct DetectionResult {
    ...
    std::vector<PredictResult> result;  // 预测结果
};
...
class DetectionPredictor {
   ...
   public:
    std::shared_ptr<Detection> _predictor;
    ...
    //StopWatch stopwatch;  //老代码中的一个类，未加入
   ...
   private:
    void work() 
    {
        ...
        {
            _can_predict = false;
            // Use non-blocking take to improve performance
            if (_queue.TryTake(frame)) 
            {
                _result->org_frame = frame.clone();  //获取图像
                _predictor->inference(_result->org_frame);   //AI推理

                if(AI_flag)
                {
                    _predictor->drawBox(_result->org_frame);  //AI识别画框操作
                    
                    imshow("ai",_result->org_frame);
                    cv::imwrite("ai00.jpg",_result->org_frame);
                }
                _result->result = _predictor->getPredictResult();
                com_predictor_results.clear();  //清空上次识别结果
                com_predictor_results = _predictor->getPredictResult();  //获取本次识别结果

                _is_predicted = true;
            }
            _can_predict = true;
            //3.18延时函数删掉了 不知道会不会有影响--------------------3.30确实有影响操你妈的
   ...
   public:
    ...
    int init(std::string model_config_path) 
    {
        ...
        //LOG_I << "Model Config Path:" << model_config_path
              //<< LOG_END;

        _predictor = std::make_shared<Detection>(model_config_path);
    ...
    int start() 
    {
        ...
        _can_predict = true;
        _is_predicted = false;
        _worker_thread = std::thread(&DetectionPredictor::work, this);
    ...
    {
        if (_can_predict && _queue.Size() < 1) 
        {
            _queue.Put(frame_wraper);
        }
    };

     std::vector<PredictResult> getResult() 
     {
        if (_is_predicted) 
        {
            _is_predicted = false;
        ...
        return std::vector<PredictResult>();
    };
